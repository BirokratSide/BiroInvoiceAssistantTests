...
{
    /// <summary>
    /// GConv contains static methods for transforming one object into
    /// another - for objects that are too complex to be typecasted.
    /// </summary>
	public static class GDataTypeConverter
    {
        ...
        public static object StrToDbNull(string data)
        {
            return (data == string.Empty || data == null) ? System.DBNull.Value : (object)data;
        }
        #endregion
        #region // sql - net - bool //
        ...
        #region // net type //
        public static int IntParse(string data, int defaultValue = -1)
        {
            int result = 0;
        ...
        public static long LongParse(string data, long defaultValue = -1)
        {
            long result = 0;
        ...
        public static bool ObjectFromDataRow(object data, DataRow dr, bool include_underscores = false)
        {
            ...
            foreach (PropertyInfo dataPropertyInfo in arrPropertyInfos)
            {
                ...
                {
                    dataPropertyInfo.SetValue(data, DbTsToLong(dr[tmpName]), null);
                    continue;
                }
                if (tmpType == typeof(string))
                    dataPropertyInfo.SetValue(data, DbToStr(dr[tmpName]), null);
                else if (tmpType == typeof(bool))
                    dataPropertyInfo.SetValue(data, DbToBln(dr[tmpName]), null);
                else if (tmpType == typeof(Byte))
                    dataPropertyInfo.SetValue(data, DbToByte(dr[tmpName]), null);
                else if (tmpType == typeof(Byte?))
                    dataPropertyInfo.SetValue(data, DbToByteNull(dr[tmpName]), null);
                else if (tmpType == typeof(Int16))
                    dataPropertyInfo.SetValue(data, DbToShort(dr[tmpName]), null);
                else if (tmpType == typeof(Int16?))
                    dataPropertyInfo.SetValue(data, DbToShortNull(dr[tmpName]), null);
                else if (tmpType == typeof(Int32))
                    dataPropertyInfo.SetValue(data, DbToInt(dr[tmpName]), null);
                else if (tmpType == typeof(Int32?))
                    dataPropertyInfo.SetValue(data, DbToIntNull(dr[tmpName]), null);
                else if (tmpType == typeof(Int64))
                    dataPropertyInfo.SetValue(data, DbToLong(dr[tmpName]), null);
                else if (tmpType == typeof(Int64?))
                    dataPropertyInfo.SetValue(data, DbToLongNull(dr[tmpName]), null);
                else if (tmpType == typeof(Single))
                    dataPropertyInfo.SetValue(data, DbToFloat(dr[tmpName]), null);
                else if (tmpType == typeof(Single?))
                    dataPropertyInfo.SetValue(data, DbToFloatNull(dr[tmpName]), null);
                else if (tmpType == typeof(Double))
                    dataPropertyInfo.SetValue(data, DbToDbl(dr[tmpName]), null);
                else if (tmpType == typeof(Double?))
                    dataPropertyInfo.SetValue(data, DbToDblNull(dr[tmpName]), null);
                else if (tmpType == typeof(Decimal))
                    dataPropertyInfo.SetValue(data, DbToDec(dr[tmpName]), null);
                else if (tmpType == typeof(Decimal?))
                    dataPropertyInfo.SetValue(data, DbToDecNull(dr[tmpName]), null);
                else if (tmpType == typeof(DateTime))
                    dataPropertyInfo.SetValue(data, DbToDt(dr[tmpName]), null);
                else if (tmpType == typeof(DateTime?))
                    dataPropertyInfo.SetValue(data, DbToDtNull(dr[tmpName]), null);
                else if (tmpType == typeof(Guid))
                    dataPropertyInfo.SetValue(data, DbToGid(dr[tmpName]), null);
                else
                {
